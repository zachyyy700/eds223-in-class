...
```{r}
# Update name to clarify it's a polygon
park_polygon <- boundary

# Cast polygon border to line
park_border <- park_polygon %>%
...
```{r}
template_raster_low <- terra::rast(ext(park_polygon), resolution = 0.05, crs = st_crs(park_polygon))

template_raster_high <- terra::rast(ext(park_polygon), resolution = 0.01, crs = st_crs(park_polygon))
...
```{r}
# Aggregate by capacity, use sum for total capacity
...
# Plants appear red, useful for gauging plant health
map2 <- tm_shape(L7) +
  tm_rgb(col = tm_vars(c(4,3,2),  multivariate = TRUE)) +
  tm_title(text = "NIR, red, green")

# Water/wet soil stand out, useful for monitoring floods
map3 <- tm_shape(L7) +
...
map2 <- tm_shape(L7) +
  tm_rgb(col = tm_vars(c(4,3,2),  multivariate = TRUE)) +
  tm_title("NIR, red, green")

map3 <- tm_shape(L7) +
  tm_rgb(col = tm_vars(c(5,4,2), multivariate = TRUE)) +
  tm_title("SWIR, NIR, green")

map4 <- tm_shape(ndvi_rast) +
  tm_raster(col.scale = tm_scale_continuous(values = 'brewer.rd_yl_gn'),
            col.legend = tm_legend(title = "NDVI", orientation = 'portrait', position = c('right', 'center'))) +
  tm_title("NDVI")

map5 <- tm_shape(ndwi_rast) +
  tm_raster(col.scale = tm_scale_continuous(values = 'brewer.blues'),
            col.legend = tm_legend(title = "NDWI", orientation = 'portrait', position = c('right','center'))) +
  tm_title("NDWI")
...
```{r}
# NDMI: measures moisture in plants. Higher values = more water content in vegetation
# define NDMI as the normalized difference between NIR and SWIR

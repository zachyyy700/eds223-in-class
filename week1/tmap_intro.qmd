...
```{r}
# Plot world_vector, a spatial object of country boundaries
...
```{r}
tm_shape(world_vector) + # Lay down first spatial object
  tm_polygons() +
tm_shape(world_cities) + # Place second spatial object on top
  tm_dots()
...
tm_shape(world_elevation) + # Elevation raster data
  tm_raster() +
tm_shape(world_vector) + # Countries vector data
  tm_borders() +
tm_shape(world_cities) + # Cities vector data
  tm_dots(fill = 'red') +
...
```{r}
tm_shape(world_cities) + # Spatial extent of world_cities will apply to next layers
  tm_dots() +
  tm_text("name") +
...
tm_shape(world_vector) +
  tm_borders() +
  tm_shape(world_cities, is.main = TRUE) + # Set as main shape
  tm_dots() +
  tm_text("name")
...
```{r}
# Use a dataset to set the map extent
tm_shape(world_elevation, bbox = world_cities) + # world_cities extent
  tm_raster() +
tm_shape(world_vector) +
  tm_borders()
...
```{r}
# Use Nominatim string to set bbox
tm_shape(world_elevation, bbox = "Switzerland") +
  tm_raster() +
tm_shape(world_vector) +
  tm_borders()
...
tm_shape(world_vector) +
  tm_polygons(fill = 'blue', col = 'red', lwd = 10)
...
```{r}
# Fill polygon depending on a variable
tm_shape(world_vector) +
...
tm_shape(world_vector) +
  tm_polygons(fill = 'life_expectancy',
              fill.scale = tm_scale(values = c('white', 'purple')),
              fill.legend = tm_legend(title = "Life Expectancy (years)", position = c(1.5,2)))
...
```{r}
tm_shape(world_vector, bbox = 'Africa') +
  tm_polygons(fill = "life_expectancy",
...
tm_shape(world_vector) +
  ...
  tm_title(text = 'Global Income', frame = TRUE, frame.color = 'blue') +
  tm_layout(style = 'beaver', outer.bg.color = '#f2ff49') 
...
```{r}
map <- tm_shape(world_vector) +
  tm_polygons(fill = "gdp_per_cap")

tmap_save(map, here::here("week1", "tmap-example.png")) # Currently throws error
